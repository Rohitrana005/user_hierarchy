    ...
    $('body').on('click','#bell-notify',function(){
							            ...
							            for (i = 0; i < data.length; i++) 
								           {
										    ...
										    if(data[i].module=='vendorproject' || data[i].module=='myproject')
											{
		 ...
		 function notifiycounter()
		 {
			$.ajax({
		 		 ...
		 		 setInterval(function(){notifiycounter()}, 600000);	
    ...
    if ('Notification' in window) {
         Notification.requestPermission(function(permission)
         {
             if (permission === 'granted') 
              {
                   // create notification
                   var note = new Notification('Designing Solutions Management', {
